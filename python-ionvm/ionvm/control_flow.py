...
from ionvm_rust import Pattern
...
from ionvm_rust import Instruction
...
class Condition:
    ...
    def from_instructions(
        cls, instructions: List[Instruction], result_reg: int
    ) -> "Condition":
        """
        ...
        """
        return cls(
            "instructions", {"instructions": instructions, "result_reg": result_reg}
        )
...
class IfElseBuilder:
    ...
    def if_condition(self, condition_reg: int):
        ...
        if self.conditions:
            raise ValueError(
                "if_condition() called when if-else chain already started. Use elif_condition() instead."
            )
    ...
    def if_condition_lazy(
        self, condition_instructions: List[Instruction], result_reg: int
    ):
        """Start an if block with lazy-evaluated condition instructions."""
        if self.conditions:
            raise ValueError(
                "if_condition_lazy() called when if-else chain already started. Use elif_condition_lazy() instead."
            )

        self.conditions.append(
            Condition.from_instructions(condition_instructions, result_reg)
        )
        self.current_block = []
        self.blocks.append(self.current_block)
    ...
    def add_instruction(self, instruction: Instruction):
        ...
        if self.current_block is None:
            raise ValueError(
                "add_instruction() called without starting a condition block"
            )
    ...
    def build(self) -> List[Instruction]:
        ...
        for i, (condition, block) in enumerate(zip(self.conditions, self.blocks)):
            ...
            condition_jump = Instruction.jump_if_false(
                condition_reg, 0
            )  # Placeholder offset
        ...
        for patch_index, target_type, target_index in jump_patches:
            ...
            if instruction.opcode == "jump_if_false":
                instructions[patch_index] = Instruction.jump_if_false(
                    instruction.args[0], offset
                )
...
class WhileThenElseBuilder:
    ...
    def __init__(self):
        ...
        self.continue_points: List[
            int
        ] = []  # Instruction indices where continues occur
    ...
    def while_condition_reg(self, condition_reg: int):
        ...
        if self.condition is not None:
            raise ValueError(
                "while_condition_reg() called when while loop already started"
            )
    ...
    def build(self) -> List[Instruction]:
        ...
        instructions[condition_jump_index] = Instruction.jump_if_false(
            condition_reg, condition_offset
        )
...
def build_while_then_else(
    builder_func: Callable[[WhileThenElseBuilder], None],
) -> List[Instruction]:
    """
