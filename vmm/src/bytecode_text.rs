...
pub fn function_to_text(func: &Function) -> String {
    ...
    writeln!(
        result,
        "function {} (arity: {}, extra_regs: {}) {{",
        name, func.arity, func.extra_regs
    )
    .unwrap();
...
pub fn parse_instruction(line: &str) -> Result<Instruction, BytecodeTextError> {
    if line.starts_with("OBJECT_INIT") {
        ...
        let reg_part = parts
            .next()
            .ok_or(BytecodeTextError::InvalidFormat(
                "Missing register in OBJECT_INIT".to_string(),
            ))?
            .trim();
        let kvs_part = parts
            .next()
            .ok_or(BytecodeTextError::InvalidFormat(
                "Missing kvs in OBJECT_INIT".to_string(),
            ))?
            .trim();
        let dst = parse_register(reg_part)?;
        let kvs_str = kvs_part
            .trim_start_matches('{')
            .trim_end_matches('}')
            .trim();
        let mut kvs = Vec::new();
        if !kvs_str.is_empty() {
            for pair in kvs_str.split(',') {
                ...
                let key = kv
                    .next()
                    .ok_or(BytecodeTextError::InvalidFormat(
                        "Missing key in OBJECT_INIT kv pair".to_string(),
                    ))?
                    .trim()
                    .to_string();
                let val = kv
                    .next()
                    .ok_or(BytecodeTextError::InvalidFormat(
                        "Missing value in OBJECT_INIT kv pair".to_string(),
                    ))?
                    .trim();
    ...
    if parts.is_empty() {
        return Err(BytecodeTextError::InvalidFormat(
    ...
    match parts[0] {
        "LOAD_CONST" => {
            if parts.len() != 3 {
                return Err(BytecodeTextError::InvalidFormat(
                    "LOAD_CONST requires 2 arguments".to_string(),
                ));
            }
        ...
        "MOVE" => {
            if parts.len() != 3 {
                return Err(BytecodeTextError::InvalidFormat(
                    "MOVE requires 2 arguments".to_string(),
                ));
            }
        ...
        "ADD" => {
            if parts.len() != 4 {
                return Err(BytecodeTextError::InvalidFormat(
                    "ADD requires 3 arguments".to_string(),
                ));
            }
            let dst = parse_register(parts[1].trim_end_matches(','))?;
        ...
        "SUB" => {
            if parts.len() != 4 {
                return Err(BytecodeTextError::InvalidFormat(
                    "SUB requires 3 arguments".to_string(),
                ));
            }
            let dst = parse_register(parts[1].trim_end_matches(','))?;
            let a = parse_register(parts[2].trim_end_matches(','))?;
        ...
        "MUL" => {
            if parts.len() != 4 {
                return Err(BytecodeTextError::InvalidFormat(
                    "MUL requires 3 arguments".to_string(),
                ));
            }
            let dst = parse_register(parts[1].trim_end_matches(','))?;
            let a = parse_register(parts[2].trim_end_matches(','))?;
        ...
        "DIV" => {
            if parts.len() != 4 {
                return Err(BytecodeTextError::InvalidFormat(
                    "DIV requires 3 arguments".to_string(),
                ));
            }
            let dst = parse_register(parts[1].trim_end_matches(','))?;
            let a = parse_register(parts[2].trim_end_matches(','))?;
        ...
        "RETURN" => {
            if parts.len() != 2 {
                return Err(BytecodeTextError::InvalidFormat(
                    "RETURN requires 1 argument".to_string(),
                ));
            }
        ...
        "JUMP" => {
            if parts.len() != 2 {
                return Err(BytecodeTextError::InvalidFormat(
                    "JUMP requires 1 argument".to_string(),
                ));
            }
            let offset = parts[1]
                .parse::<isize>()
...
fn parse_register(s: &str) -> Result<usize, BytecodeTextError> {
    if !s.starts_with('r') {
        return Err(BytecodeTextError::InvalidFormat(format!(
...
mod tests {
    ...
    fn test_value_to_text() {
        assert_eq!(
            value_to_text(&Value::Primitive(Primitive::Number(42.0))),
            "42"
        );
        assert_eq!(
            value_to_text(&Value::Primitive(Primitive::Boolean(true))),
            "true"
        );
        assert_eq!(
            value_to_text(&Value::Primitive(Primitive::Atom("hello".to_string()))),
            "'hello'"
        );
    ...
    fn test_parse_instruction() {
        ...
        assert!(
            matches!(instr, Instruction::LoadConst(0, Value::Primitive(Primitive::Number(n))) if n == 42.0)
        );
